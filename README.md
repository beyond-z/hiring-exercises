...
To interact with the app in the console, run this:
    ...
    docker-compose exec career-mgr-hiring rails c

As you may have guessed, running the app through Docker does have the drawback of having to pre-pend "docker-compose exec career-mgr-hiring" to every rails/rake command. If you're in a unix-y environment (and Mac OS X counts) you can add this alias to your ~/.bash-profile file:

    alias career="docker-compose exec career-mgr-hiring"
...
Copy them to the `tmp`, then:
    ...
    docker-compose exec career-mgr-hiring rake postal:load
...
Whenever you pull in changes from upstream, you should run the following commands in the root directory of the app: 
    ...
    docker-compose build career-mgr-hiring
    docker-compose stop career-mgr-hiring
    docker-compose up -d --force-recreate career-mgr-hiring
    docker-compose up -d
    
    docker-compose exec career-mgr-hiring rake db:migrate
   
    docker-compose exec career-mgr-hiring rspec
...
If you'd like to reset the database to the original seed data (with example employers, industries, and interests), you can re-run this command at any time:
    ...
    docker-compose exec career-mgr-hiring rake db:seed

...or go all out with this one:

    docker-compose exec career-mgr-hiring rake db:drop db:create db:schema:load db:init db:seed

## Troubleshooting

* `HTTPError Could not fetch specs from https://rubygems.org/` - Restart docker.
* If rspec fails with a `PendingMigrationError`, run this: `docker-compose exec career-mgr-hiring rake db:migrate RAILS_ENV=test` (sometimes you may need to run it more than once)
...
You can open a dedicated terminal window and have tests run automatically as you code:
    ...
    docker-compose exec career-mgr-hiring guard
